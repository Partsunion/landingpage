...
const sessionTtlSeconds = Number.parseInt(process.env.MAINTENANCE_SESSION_TTL_SECONDS ?? '43200', 10);
...
const staticHandlerOptions = { public: publicDirectory, cleanUrls: true, directoryListing: false };
...
function verifyPassword(password) {
    ...
    const supplied = scryptSync(password, salt, 64, { N: 16384, r: 8, p: 1, maxmem: 64 * 1024 * 1024 });
...
async function readForm(request) {
    return await new Promise((resolveForm, rejectForm) => {
        ...
        request.on('end', () => resolveForm(new URLSearchParams(Buffer.concat(chunks).toString('utf8'))));
    ...
    try {
        ...
        if (gated && requestUrl.pathname === '/__maintenance/unlock') {
            ...
            if (rate.lockedUntil > Date.now()) {
                ...
                sendHtml(response, 429, maintenancePage({ error: 'Zu viele Fehlversuche. Bitte später erneut versuchen.' }), seconds);
                return;
            }
            ...
            if (password.length > 128 || !verifyPassword(password)) {
                ...
                sendHtml(response, 401, maintenancePage({ error: 'Das Passwort ist nicht korrekt.', next }));
                return;
            }
        ...
        if (gated && !hasValidSession(request)) {
            ...
            const html = maintenancePage({ next: safeReturnPath(requestUrl.pathname + requestUrl.search) });
...
server.listen(port, '0.0.0.0', () => {
    console.log(`Partsunion web server listening on port ${port}; maintenance=${maintenanceEnabled ? 'enabled' : 'disabled'}`);
});
