...
export function berlinAppointment(day: string, time: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day) || !appointmentTimes.includes(time)) throw new Error("Bitte wähle einen gültigen Terminwunsch.");
  const utc = new Date(`${day}T${time}:00Z`);
  const zone = new Intl.DateTimeFormat("en-US", { timeZone: "Europe/Berlin", timeZoneName: "shortOffset" }).formatToParts(utc).find((part) => part.type === "timeZoneName")?.value;
  return `${day}T${time}:00${zone === "GMT+2" ? "+02:00" : "+01:00"}`;
}

export function validAppointment(iso: string, now = new Date()): boolean {
  const day = iso.slice(0, 10);
  const time = iso.slice(11, 16);
  return appointmentDays(now).includes(day) && appointmentTimes.includes(time) && berlinAppointment(day, time) === iso && new Date(iso) > now;
}
